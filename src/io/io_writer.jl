...
function preprocess_daily_outputs(
    ...
    gpu_results = (
        ...
        swe_summed = dropdims(sum(swe_gpu .* AreaFract_gpu, dims=3), dims=3),
        snow_coverage = dropdims(sum(snow_coverage_gpu .* AreaFract_gpu, dims=3), dims=3),
        snow_melt = dropdims(sum(snow_melt_gpu .* AreaFract_gpu, dims=3), dims=3)
    )
    
    # Properly normalize intensive fractional variables back to Snow-Covered-Area averages
    # to avoid artificial mass-deflation natively generated by `AreaFract` reduction
    coverage_norm = max.(gpu_results.snow_coverage, eltype(gpu_results.snow_coverage)(1e-6))
    
    return merge(gpu_results, (
        snow_albedo = sum_with_nan_handling(snow_albedo_gpu .* snow_coverage_gpu .* AreaFract_gpu, 3) ./ coverage_norm,
        snow_surf_temp = sum_with_nan_handling(snow_surf_temp_gpu .* snow_coverage_gpu .* AreaFract_gpu, 3) ./ coverage_norm
    ))
end